...
function makeEntry(overrides: Partial<IStatusHistoryEntryDTO> = {}): IStatusHistoryEntryDTO {
  return {
...
function makeHeatmapDay(date: string, overrides: Partial<IHeatmapDay> = {}): IHeatmapDay {
  return {
    date,
    dominantSignalType: HeartbeatSignalType.PRESENCE,
    totalCount: 10,
...
describe('HistoryLedger', () => {
  const entries: IStatusHistoryEntryDTO[] = [
    ...
    makeEntry({ id: 'e3', signalType: 'check_failed', httpStatusCode: 500, errorMessage: 'server error' }),
  ];
  ...
  it('search input filters entries', () => {
    render(<HistoryLedger entries={entries} />);
    const searchInput = screen.getByTestId('ledger-search').querySelector('input')!;
